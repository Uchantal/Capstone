      ...
      <div className="flex-shrink-0 bg-[#F9F7F4] border-b border-surface-border px-4 py-3">
        <div className="flex flex-wrap gap-x-6 gap-y-3 items-start">
          <div className="flex-1 min-w-[220px]">
            ...
            <p className="text-text-muted text-[10px] mb-1">Who is the audience and why do your design choices suit them? Write at least one sentence.</p>
            <textarea
              value={reasoning}
          ...
          <div className="flex-shrink-0">
            <p className="text-[10px] font-semibold text-text-secondary uppercase tracking-widest mb-1">Checklist</p>
            <p className="text-text-muted text-[10px] mb-1.5">Tick each item only when it is genuinely true for your poster.</p>
            <div className="space-y-1.5">
              {CHECKLIST.map(item => (
                <label key={item.id} className="flex items-start gap-2 cursor-pointer">
                  ...
                  />
                  <span className={`text-xs leading-relaxed ${checked.has(item.id) ? 'text-text-primary' : 'text-text-secondary'}`}>
                    {item.text}
                  </span>
                </label>
              ))}
