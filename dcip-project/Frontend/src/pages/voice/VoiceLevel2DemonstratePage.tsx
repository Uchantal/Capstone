...
export default function VoiceLevel2DemonstratePage() {
    ...
    if (next >= DEMO_SCALE.length) {
      ...
      if (didPass) {
        if (!isPreviewMode) {
          ...
          savePortfolioItem({ discipline: 'voice', title: 'Voice Level 2 Demonstration', fileType: 'result', fileData: 'Passed', durationMinutes: 0 }).catch(() => {})
        }
        reload()
      }
  ...
  const runLoop = useCallback(() => {
    ...
    const status = getPitchStatus(freq, DEMO_SCALE[promptIdxRef.current].freq, DEMO_TOLERANCE)
      ...
      <MainLayout>
        <div className="max-w-6xl mx-auto px-6 md:px-10 lg:px-16 py-8">
          <div className="flex items-center gap-2 text-xs text-text-muted mb-5">
            ...
            <span>/</span><span>Level 2 Demonstration</span>
          </div>
          ...
          <div className="space-y-3">
            {passed ? (
              <button
                onClick={() => navigate('/voice/level-3')}
  ...
  return (
    <MainLayout>
      <div className="max-w-6xl mx-auto px-6 md:px-10 lg:px-16 py-8">
        ...
        <h1 className="text-text-primary font-bold text-2xl mb-1">Level 2: Demonstration</h1>
        <p className="text-text-secondary text-sm mb-6">
          Sing each note of the C major scale without hearing it first. Hold steady for 1.5 seconds to advance.
          You need {REQUIRED_CORRECT} of {DEMO_SCALE.length} correct to pass.
        </p>

        <div className="bg-white border border-surface-border rounded-2xl p-6 mb-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-text-muted text-xs uppercase tracking-wide">
              Note {promptIdx + 1} of {DEMO_SCALE.length}
            </p>
