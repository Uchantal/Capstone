    ...
    >
      ...
      <div className="relative group">
        <button onClick={() => onToolChange('brush')} className={toolBtnCls(activeTool === 'brush')}>
          {/* Standard pencil icon */}
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/>
            <path d="m15 5 4 4"/>
          </svg>
        </button>
        <Tooltip label="Pencil / Brush" />
      </div>
      ...
      {/* Shape fill/outline toggle — only when rect or circle active */}
      {isShapeTool && (
        <>
          <Divider />
          <div className="relative group">
      ...
      {/* ── Brush size — shows current px value ─────────────── */}
      <div className="relative group">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-2">
            {BG_PRESETS.map(p => (
              ...
              >
                <span
                  className="w-7 h-7 rounded border border-surface-border"
