...
export default function GraphicDesignToolbar({
  ...
  return (
    // Outer wrapper: provides positioning context for the scroll-shadow gradient
      ...
      >
          ...
          <div style={popupStyle} className={`${popupBase} p-3 w-64 max-h-[80vh] overflow-y-auto`}>
            ...
            <div className="grid grid-cols-3 gap-2">
              {PICKER_ITEMS.map(item => {
                ...
                return (
                  <button key={key} onClick={() => pickShape(item)} className="flex flex-col items-center justify-start gap-1 pt-2 pb-1.5 px-1 rounded-lg bg-surface-warm hover:bg-white hover:shadow-sm cursor-pointer">
                    <div style={{ width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {item.kind === 'rect'   && <div style={{ width: 36, height: 24, background: '#C8960C', borderRadius: 2 }} />}
                      {item.kind === 'circle' && <div style={{ width: 36, height: 36, background: '#C8960C', borderRadius: '50%' }} />}
                      {item.kind === 'shape'  && <svg width={40} height={40} viewBox="0 0 100 100" preserveAspectRatio="none">{SHAPE_SVG[item.type]('#C8960C')}</svg>}
                    </div>
        ...
        {openPopup === 'background' && (
          <div style={popupStyle} className={`${popupBase} p-4 w-64`}>
            ...
            <label className="block mb-3 cursor-pointer">
              <input type="color" value={bgColour} onChange={e => onBgColourChange(e.target.value)} className="w-full h-10 cursor-pointer rounded-lg border border-surface-border" style={{ padding: 2 }} />
            </label>
            <div className="flex flex-wrap gap-2">
              {BG_COLORS.map(c => (
                <button key={c} onClick={() => { onBgColourChange(c); setOpenPopup(null) }} title={c}
                  className={`w-7 h-7 rounded-full border-2 flex-shrink-0 transition-all ${bgColour === c ? 'border-primary scale-110' : 'border-transparent hover:border-text-primary/30'}`}
        ...
        {openPopup === 'colour' && hasSelected && (
          <div style={popupStyle} className={`${popupBase} p-4 w-64`}>
            ...
            <label className="block mb-3 cursor-pointer">
              <input type="color" value={colour ?? '#C8960C'} onChange={e => onColourChange(e.target.value)} className="w-full h-10 cursor-pointer rounded-lg border border-surface-border" style={{ padding: 2 }} />
            </label>
            <div className="flex flex-wrap gap-2">
              {ELEMENT_COLORS.map(c => (
                <button key={c} onClick={() => { onColourChange(c); setOpenPopup(null) }} title={c}
                  className={`w-7 h-7 rounded-full border-2 flex-shrink-0 transition-all ${colour === c ? 'border-primary scale-110' : 'border-transparent hover:border-text-primary/30'}`}
          ...
          <div style={popupStyle} className={`${popupBase} p-3 w-56`}>
            ...
            <div className="flex flex-col gap-1">
              {templates.map(t => {
                ...
                return (
                  <button key={t.id} onClick={() => { onTemplateChange(t); setOpenPopup(null) }}
                    className={`flex items-center gap-3 px-2 py-2 rounded-lg text-left transition-colors border ${isActive ? 'bg-primary/10 border-primary/30' : 'hover:bg-surface-warm border-transparent'}`}
                  >
        ...
        {openPopup === 'fontsize' && isText && (
          <div style={popupStyle} className={`${popupBase} p-4 w-56`}>
            ...
            <div className="flex items-center gap-1 mb-3">
              <button onClick={() => { const s = Math.max(8, localFontSz - 1); setLocalFontSz(s); onFontSizeChange(s) }}
                className="w-7 h-7 flex items-center justify-center rounded-lg border border-surface-border text-text-secondary hover:bg-surface-warm transition-colors text-sm font-bold">-</button>
              <input type="number" min={8} max={200} value={localFontSz}
                onChange={e => { const s = Math.max(8, Math.min(200, parseInt(e.target.value) || 8)); setLocalFontSz(s); onFontSizeChange(s) }}
              ...
              <button onClick={() => { const s = Math.min(200, localFontSz + 1); setLocalFontSz(s); onFontSizeChange(s) }}
                className="w-7 h-7 flex items-center justify-center rounded-lg border border-surface-border text-text-secondary hover:bg-surface-warm transition-colors text-sm font-bold">+</button>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {FONT_SIZE_PRESETS.map(s => (
                <button key={s} onClick={() => { setLocalFontSz(s); onFontSizeChange(s); setOpenPopup(null) }}
                  className={`text-xs px-2 py-1 rounded-lg border transition-colors ${localFontSz === s ? 'border-primary bg-primary/5 text-primary font-semibold' : 'border-surface-border text-text-secondary hover:border-primary/40'}`}>
                  {s}px
                </button>
              ))}
            </div>
